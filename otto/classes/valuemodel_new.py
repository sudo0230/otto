...
class ValueModel_new(nn.Module):
    ...
    def __init__(self,
        ...
        if FC_layers > 0:
            for i in range(FC_layers+2):
                ...
                if i == 0:
                    setattr(self, layer_name, nn.Linear(in_features, FC_units))
                elif i == FC_layers + 1:
                    setattr(self, layer_name, nn.Linear(FC_units, 1))
                else:
                    setattr(self, layer_name, nn.Linear(FC_units, FC_units))
    ...
    def forward(self,x):
        ...
        for i in range(self.FC_layers + 2):
            if i == (self.FC_layers + 1):
                layer = getattr(self, f'dense{i+1}')
                x=layer(x)
            else:
                layer = getattr(self, f'dense{i+1}')
    ...
    def train_step(self, x, y, optimizer, loss_fxn, augment=False):
        """A training step.
