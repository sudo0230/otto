...
class TrainingEnv(SourceTracking):
    ...
    def choose_action_from_statep(self, model, statep=None, sym_avg=False):  # statep: (Nactions, Nhits)
        ...
        model_only, _, _ = model
        action_chosen, _ = self._value_policy_from_statep(model=model_only, statep=statep, sym_avg=sym_avg)
    ...
    def transitions(self):
        ...
        for action in range(self.Nactions):
            ...
            p_source_ = p_source_[None, :] * p_evidence
        ...
        # print("statep :", statep)
        # print("statep :", statep[0][0])
        # print("type statep :", type(statep[0]))
    ...
    def get_state_value(self, model, state):
        ...
        model_only,_,_ = model
        print("modellll",type(model_only(input)))
        value = model_only(input)
    ...
    def get_target(self, modelvalue, modelaction, statep):
        ...
        inputp = torch.tensor(inputp, dtype=torch.float32)
        inputp = torch.reshape(inputp,
                            shape=tuple([inputpshape[0] * inputpshape[1] * inputpshape[2]] + list(inputpshape[3:])))
        assert inputp.shape == tuple([batch_size * self.Nactions * self.Nhits] + list(ishape))

        # calculating the target
        values_next = modelvalue[0](inputp)
        ...
        # sum over hits
        probp = torch.tensor(probp, dtype=torch.float32)
        probp_tens = probp.clone().detach()
        expected_value = 1.0 + torch.sum(probp_tens * values_next, dim=-1)
        assert expected_value.shape == tuple([batch_size] + [self.Nactions])

        if modelaction is not None:
            expected_value_for_action = 1.0 + torch.sum(probp_tens * values_next_for_action, dim=-1)
            assert expected_value_for_action.shape == tuple([batch_size] + [self.Nactions])

        # reduce over best action
        if modelaction is not None:
            # get action indices
            chosen_action = torch.argmin(expected_value_for_action, dim=1)  # (batch_size,)
            # get array of indices as required for gather_nd
            ind = torch.stack((torch.arange(batch_size, dtype=torch.int), chosen_action)).transpose(0, 1)  # (batch_size, 2)
            # get expected_value[action_indices]
            target = torch.gather(expected_value, index=ind)  # (batch_size,)
            target = target[..., None]  # (batch_size, 1)
        else:
            # min over actions
            target = torch.min(expected_value, dim=1).values.unsqueeze(1)  # (batch_size, 1)
    ...
    def _value_policy_from_statep(self, model, statep, sym_avg=True):  # statep: (Nactions, Nhits)
        ...
        inputpshape = inputp.shape
        # print("action chosen1")
        # print(f"Input shape before reshape : {inputpshape}")
        # print("Change to : ",tuple([inputpshape[0] * inputpshape[1] * inputpshape[2]] + list(inputpshape[3:])))
        
        #with tf.device('/cpu:0'):
        inputp = torch.tensor(inputp, dtype=torch.float32)
        inputp = torch.reshape(inputp, shape=tuple([inputpshape[0] * inputpshape[1] * inputpshape[2]] + list(inputpshape[3:])))  # (1 * Nactions * Nhits, inputshape)
        assert inputp.shape == tuple([1 * self.Nactions * self.Nhits] + list(ishape))

        # print(inputp)

        # values_next = model(inputp, sym_avg=sym_avg)  # (1 * Nactions*Nhits, 1)
        values_next = model(inputp)
        values_next = values_next.detach().cpu().numpy()  # Detach, move to CPU, and convert to NumPy
        # print("action chosen3")
        assert values_next.shape == tuple([1 * self.Nactions * self.Nhits] + [1])
        probp = torch.tensor(probp, dtype=torch.float32)    

        values_next = torch.tensor(values_next, dtype=torch.float32)
        values_next = torch.reshape(values_next, shape=(inputpshape[0], inputpshape[1], inputpshape[2],))  # (1, Nactions, Nhits)
        assert values_next.shape == tuple([1] + [self.Nactions] + [self.Nhits])
        assert values_next.shape == probp.shape
        

        probp_tens = probp.clone().detach()
        expected_value = 1.0 + torch.sum(probp_tens * values_next, dim=2)  # sum over hits: (1, Nactions)
        assert expected_value.shape == tuple([1] + [self.Nactions])

        action_chosen = np.argmin(expected_value.squeeze()) 
             
        return action_chosen, expected_value.squeeze()
