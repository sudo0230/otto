...
Parameters of the script are:
    ...
    1. I added a spawn context command in compute_stats function (not necessary)
    2. I added gpu physical devices and memory growth in main function (not necessary)
    3. I added 3 flags in compute_stats function to narrow down the line of error (CUDA Initialization error))
    4. (13th Nov) : Found the fix for cuda initialization error. Specify some particular trouble causing tasks to CPU keeping rest of the code same
    5. I specified cpu processing for 'build_new_model' function
    6. I specified cpu processing for '_value_policy_for_statep' (internal function) in training.py file
"""
import os
...
from otto.classes.training_new import TrainingEnv as env
# from otto.classes.valuemodel import ValueModel, reload_model
from otto.classes.valuemodel_new import ValueModel_new, reload_model_new
...
# def setup_gpu():
#     # Optional: Set threads for intra and inter operations to avoid issues
#     tf.config.threading.set_intra_op_parallelism_threads(1)  
#     tf.config.threading.set_inter_op_parallelism_threads(1)

#     """ Function to configure the GPU memory growth (for each process). """
...
def build_new_model(
    ...
    # Loss function I have defined here
    loss_fxn = nn.MSELoss()

    # Optimizer I am using
    optimizer = optim.Adam(model.parameters(), lr=0.001)

    return model, loss_fxn, optimizer

def build_new_gpu_model(
        Ndim,
        FC_layers,
        FC_units,
        inputshape,
...
def param2subtitle(env, model):
    ...
    Nweights = np.sum([np.prod(w.shape) for w in model[0].state_dict().values() if hasattr(w, 'shape')])
...
def compute_stats(Nepisodes, policy, parallel=True):
    ...
    # print("yyyy",mean,tot,pdf_t,bin_t)
...
def train_model(eps_floor, eps_0, eps_decay, max_it, ref_stats):
    ...
    while it <= max_it:
        ...
        if it % SAVE_MODEL_EVERY == 0:
            ...
            MYMODEL[0].save_model(model_path)
        ...
        ###################### EVALUATE PERFORMANCE OF CURRENT MODEL ON CPU using MYCPUMODEL
        ...
        ###################### GENERATE EXP ON CPU USING MYCPUMODEL
        ...
        # print("training on GPU starts...")

        # TODO: convert states, statesp to input for NN and move them to GPU

        # x, _, = MYENV.states2inputs(states[batch], 0)  # (BATCH_SIZE , inputshape), (BATCH_SIZE)
        # y = get_target(statesp[batch])
        # x_gpu=torch.tensor(x, dtype=torch.float32).to("cuda")
        # y_gpu=torch.tensor(y, dtype=torch.float32).to("cuda")


        ###################### TRAINING MYGPUMODEL ON GPU
        ...
        #     x, _, = MYENV.states2inputs(states[batch], 0)  # (BATCH_SIZE , inputshape), (BATCH_SIZE)
        #     y = get_target(statesp[batch])
        #     loss = MYGPUMODEL.train_step(x, y, augment=SYM_TRAIN_ADD_DUPLICATES)  # train (model weights are updated)
        #     mean_loss += loss
        ...
        if it % UPDATE_FROZEN_MODEL_EVERY == 0:
            MYFROZENMODEL[0].load_state_dict(MYMODEL[0].state_dict())      
        ...
        for gd_step in range(N_GD_STEPS):
            ...
            # Prepare inputs and targets (move inputs to GPU)
            x, _ = MYENV.states2inputs(states[batch], 0)  # (BATCH_SIZE, inputshape), (BATCH_SIZE)
            y = get_target(statesp[batch])  # Target labels

            # Convert x and y to PyTorch tensors and move to GPU
            x_new = torch.tensor(x, dtype=torch.float32).to('cuda')
            y_new = torch.tensor(y, dtype=torch.float32).to('cuda')

            # Perform a training step on GPU
            loss = MYGPUMODEL[0].train_step(x_new, y_new, augment=SYM_TRAIN_ADD_DUPLICATES)
            mean_loss += loss
        ...
        # Sync GPU model weights to CPU model
        MYMODEL[0].load_state_dict(MYGPUMODEL[0].state_dict())
...
#     """ Function to run the workers in parallel. """
#     # with :  # Ensure the proper start method
#     with ThreadPoolExecutor(max_workers=4) as executor:
#         # List of arguments for each worker function
#         tasks = [(episode, policy, eps, memorize) for episode in range(10)]  # Adjust this as needed
#         results = list(executor.map(lambda args: worker(*args), tasks))
#         return results
...
if __name__ == '__main__':
    ...
    """ Worker function to run in parallel (use the setup_gpu function within). """
    # setup_gpu()  # Set GPU memory growth for this worker

    # # Initialize TensorFlow and GPU context in the main thread
    # tf.config.list_physical_devices('GPU')  # Forces CUDA initialization
    ...
    MYFROZENMODEL[0].load_state_dict(MYMODEL[0].state_dict())
    MYGPUMODEL = build_new_gpu_model(
            Ndim=N_DIMS,
            FC_layers=FC_LAYERS,
            FC_units=FC_UNITS,
            inputshape=MYENV.NN_input_shape,
        )
    # MYGPUMODEL.set_weights(MYMODEL.get_weights())

    if CONTINUE_TRAINING:
        print("\n*** Reloading previous weights from:", MODEL_PATH)
        oldmodel = reload_model(MODEL_PATH, inputshape=MYENV.NN_input_shape)
        MYMODEL[0].load_state_dict(oldmodel[0].state_dict())
        MYFROZENMODEL[0].load_state_dict(oldmodel[0].state_dict())
        del oldmodel
